 ...
 * UpdateInventory API: Updates the inventory after a successful placement of order.
 */
...
export default function (products) {
    ...
    let each_product = http.get(get_product_api_url + "/" + product_id, params)
    ...
    // Update Inventory by subtracting the number of units for each of the warehouses for the product
    // For now it only updates a single warehouse as the order was only placed from a single warehouse.

    inventory_arr.forEach(inventory_item => {
        let warehouse_inventory_update = {
            warehouseId: inventory_item.warehouseId,
            units: -(inventory_item.units)
        }
        let warehouse_arr = []
        warehouse_arr.push(warehouse_inventory_update);
        let productId = inventory_item.productId;
        let update_inventory_body = {
            warehouses: warehouse_arr
        }
        let update_inventory_url = get_product_api_url + "/" + productId + "/inventory";
        let update_inventory = http.post(update_inventory_url, JSON.stringify(update_inventory_body), params);
        check(update_inventory, {
            "UpdateInventory status is 200": r => r.status >= 200 && r.status < 300
        });
    });
};
